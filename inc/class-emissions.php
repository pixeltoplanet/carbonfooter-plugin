...
class Emissions {
	...
	public function get_post_emissions( $post_id ) {
		...
		if ( $value !== false && $value !== null ) {
			...
			Logger::info( 'Per-post cache miss, DB optimizer hit', array( 'post_id' => $post_id ) );
			return (float) $value;
		}
		...
		if ( $meta_val !== '' && $meta_val !== null ) {
			...
			$this->cache->set_post_payload( $post_id, $payload_from_meta );
			Logger::info( 'Per-post cache miss, meta hit', array( 'post_id' => $post_id ) );
			return (float) $meta_val;
		}

		Logger::info( 'Per-post cache miss, no data found', array( 'post_id' => $post_id ) );
		return false;
	}
	...
	public function get_post_payload( int $post_id ): ?array {
		...
		if ( $value !== false && $value !== null ) {
			$payload_from_meta = $this->build_payload_from_meta( $post_id, (float) $value );
			$this->cache->set_post_payload( $post_id, $payload_from_meta );
			Logger::info( 'Payload cache miss, DB optimizer hit', array( 'post_id' => $post_id ) );
		...
		if ( $meta_val !== '' && $meta_val !== null ) {
			$payload_from_meta = $this->build_payload_from_meta( $post_id, (float) $meta_val );
			$this->cache->set_post_payload( $post_id, $payload_from_meta );
			Logger::info( 'Payload cache miss, meta hit', array( 'post_id' => $post_id ) );
			return $payload_from_meta;
		}

		Logger::info( 'Payload cache miss, no data found', array( 'post_id' => $post_id ) );
		return null;
	}
