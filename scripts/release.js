...
function getVersionFromPluginHeader(content) {
  ...
  if (!match) throw new Error('Could not extract Version from carbonfooter.php');
  return match[1];
}
...
function main() {
  ...
  // Create staging dir
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'carbonfooter-release-'));
  ...
  if (!cmdExists('rsync')) {
    throw new Error('rsync not found. Please install rsync or adapt the script to a pure Node copy.');
  }

  const excludeArgs = exclude.map((e) => `--exclude='${e}'`).join(' ');
  execSync(`rsync -av ${excludeArgs} "${ROOT}/" "${stagingDir}/"`, { stdio: 'inherit' });
  ...
  if (!fs.existsSync(stagedBuildJs)) {
    ...
    if (!canBuild) {
      throw new Error('Cannot build: package.json not found or pnpm not installed. Run pnpm run build manually.');
    }
    ...
    execSync(`rsync -av "${sourceBuild}/" "${path.join(stagingDir, 'build')}/"`, { stdio: 'inherit' });
  }
  ...
  execSync(`cd "${tempDir}" && zip -r "${zipPath}" carbonfooter/ > /dev/null`, { stdio: 'inherit', shell: '/bin/bash' });
  ...
  if (cmdExists('unzip')) {
    try {
      ...
      execSync(`unzip -l "${zipPath}" | head -20`, { stdio: 'inherit', shell: '/bin/bash' });
    } catch {}
  }
}
